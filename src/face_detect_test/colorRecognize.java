...
public class colorRecognize {
	...
	public short[][] solveForMatrix(){
		short[][] frt_mark = new short[heightRec][widthRec];
	...
	//	out = new BufferedImage( width , height , BufferedImage.TYPE_INT_RGB );
		for ( int x = startX ; x < startX + widthRec - 1 ; x ++ ){
			for ( int y = startY ; y < startY + heightRec - 1 ; y ++ ){
				int[] pix = getXY(x , y);
				if ( facial(pix , facial ) ){
					frt_mark[y - startY][x - startX] = Imgproc.GC_FGD;
				}
				else
					frt_mark[y - startY][x - startX] = Imgproc.GC_PR_BGD;
			}
		}
		return frt_mark;
	}
